...
1. Run `FastChem` to make a chemical abundance table following the instructions described in [Wiki](https://github.com/nombac/optab/wiki/Calculating-a-chemical-abundance-table-with-FastChem).
...
3. Now execute the sample script `table.sh` located in `$OPTAB/sample/` to run `Optab`:
   ...
   $ bash table.sh
   ```
    It takes about 28min on 3.3 GHz 12-Core Intel Xeon W with 12 MPI processes.
